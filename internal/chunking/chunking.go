...
func (s *SentenceSplitter) ChunkWithMetadata(text string, metadata map[string]any) []gleann.Item {
	chunks := s.Chunk(text)
	items := make([]gleann.Item, len(chunks))
...
func (c *CodeChunker) ChunkWithMetadata(code string, metadata map[string]any) []gleann.Item {
	chunks := c.Chunk(code)
	items := make([]gleann.Item, len(chunks))
	for i, chunk := range chunks {
		meta := make(map[string]any)
