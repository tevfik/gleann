...
// --- Install/Uninstall logic (legacy non-progress versions) ---

func installPlugin(info pluginInfo) (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("user home dir: %w", err)
	}
	...
	if _, err := os.Stat(pluginDir); os.IsNotExist(err) {
		...
		if _, err := os.Stat(repoDir); os.IsNotExist(err) {
			...
			output, cloneErr := cmd.CombinedOutput()
			if cloneErr != nil {
				// Git failed - try GitHub release.
				noopProgress := make(chan string, 10)
				go func() {
				...
				owner, repo := parseGitHubURL(info.RepoURL)
				if owner != "" && repo != "" {
					if err := downloadSourceFromGitHub(owner, repo, repoDir, noopProgress); err == nil {
						close(noopProgress)
						// Continue to linking below.
					} else {
						close(noopProgress)
						return "", fmt.Errorf("git clone and release download failed: %s / %v", string(output), err)
					}
				} else {
	...
	lang := info.Language
	if strings.Contains(lang, "python") {
		return setupPythonPlugin(pluginDir, info.Name)
	} else if strings.Contains(lang, "go") {
		return setupGoPlugin(pluginDir, info.Name)
	}

	return fmt.Sprintf("Installed %s", info.Name), nil
}

func setupPythonPlugin(pluginDir, name string) (string, error) {
	venvDir := filepath.Join(pluginDir, ".venv")
